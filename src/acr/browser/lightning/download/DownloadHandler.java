...
import acr.browser.lightning.utils.WebAddress;
...
public class DownloadHandler {
		...
		if (contentDisposition == null
			...
			if (info != null) {
				...
				if (!myName.getPackageName().equals(info.activityInfo.packageName)
					...
					// someone (other than us) knows how to handle this mime type with this scheme, don't download.
	...
	static void onDownloadStartNoStream(Activity activity, String url, String userAgent,
		...
		// java.net.URI is a lot stricter than KURL so we have to encode some extra characters. 
		// Fix for b 2538060 and b 1634719
		...
		try {
			...
			// This only happens for very bad urls, we want to catch the exception here
		...
		// set downloaded file destination to /sdcard/Download. or, 
		// should it be set to one of several Environment.DIRECTORY* dirs depending on mimetype?
		...
		// let this downloaded file be scanned by MediaScanner 
		// - so that it can show up in Gallery app, for example.
		request.allowScanningByMediaScanner();
		request.setDescription(webAddress.getHost());
		
		// XXX: Have to use the old url since the cookies were stored using the old percent-encoded url.
		...
		if (mimetype == null) {
			...
			// We must have long pressed on a link or image to download it. 
			// We are not sure of the mimetype in this case, so do a head request
